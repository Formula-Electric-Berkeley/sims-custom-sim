...
def opt_mintime():
        ...
        # DAE variables: Compare with UMunich's use of gamma: TODO*********************
        gamma_xn = ca.SX.sym('gamma_xn') # longitudinal tire load transfer front/back [N]
        gamma_xs = 5000.0
        gamma_x = gamma_xn*gamma_xs

        gamma_yn = ca.SX.sym('gamma_yn') # lateral tire load transfer left/right [N]
        gamma_ys = 5000.0
        gamma_y = gamma_yn*gamma_ys
        ...
        u_s = np.array([delta_s, torque_drive_s, f_brake_s, gamma_xs, gamma_ys])

        # put all controls together
        u = ca.vertcat(delta_n, torque_drive_n, f_brake_n, gamma_xn, gamma_yn)
        ...
        ackermann = True  # if false - uses parallel steering
        if ackermann:
        ...
        # CONTROL BOUNDARIES -----------------------------------------------------------------------------------------------
        # ------------------------------------------------------------------------------------------------------------------
        #Recall:         u = (delta, torque_drive, f_brake, gamma_x, gamma_y)
        ...
        '''
        Constrain later with 
        # dae constraints from Fork and Borrelli ; these define our gammas
        Ky_constraint = -gamma_x * L * 2  - Fx * h # net torque about body frame y axis = 0
        Kx_constraint = gamma_y * (wf+wr) + Fy * h # net torque about body frame x axis = 0
        '''
        ...
        vx_min = 1.0 / vx_s                               # min. x velocity [m/s]
        vx_max = veh.max_velocity / vx_s                  # max. x velocity [m/s]
        vy_min = 1.0 / vx_s                               # min. y velocity [m/s]
        vy_max = veh.max_velocity / vx_s                  # max. y velocity [m/s]
        ...
        for k in range(N):
                ...
                w.append(Uk)
                lbw.append([delta_min, f_drive_min, f_brake_min, gamma_y_min, gamma_x_min])
                ubw.append([delta_max, f_drive_max, f_brake_max, gamma_y_max, gamma_x_max])
                w0.append([0.0] * nu)
                ...
                #ec_opt.append(0) # Xk[0] * v_s * Uk[1] * f_drive_s * dt_opt[-1]
                ...
                # path constraint: limitied engine power; TODO; put motor torque curve here
                #g.append(Xk[0] * Uk[1])
                #lbg.append([-np.inf])
                ...
                #         * veh.cg_height / ((veh.track_width_front + veh.track_width_rear) / 2) - Uk[3] * gamma_ys)
                # lbg.append([0.0])
                # ubg.append([0.0])
